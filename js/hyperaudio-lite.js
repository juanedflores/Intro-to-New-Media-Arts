...
class BasePlayer {
  ...
  attachEventListeners(instance) {
    this.player.addEventListener(
      "pause",
      instance.pausePlayHead.bind(instance),
      false,
    );
    this.player.addEventListener(
      "play",
      instance.preparePlayHead.bind(instance),
      false,
    );
  }
...
function hyperaudioPlayer(playerType, instance) {
  if (playerType) {
    ...
    console.warn(
      "HYPERAUDIO LITE WARNING: data-player-type attribute should be set on player if not native, e.g., SoundCloud, YouTube, Vimeo, VideoJS",
    );
  }
}
...
class HyperaudioLite {
  ...
  setupEventListeners(doubleClick, playOnClick) {
    ...
    this.transcript.addEventListener(
      playHeadEvent,
      this.setPlayHead.bind(this),
      false,
    );
    this.transcript.addEventListener(
      playHeadEvent,
      this.checkPlayHead.bind(this),
      false,
    );
  }
