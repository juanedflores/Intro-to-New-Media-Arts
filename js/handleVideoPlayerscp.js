...
const targetNode = document.getElementById("trigger1");
  ...
  for (const mutation of mutationsList) {
    if (mutation.attributeName === "class") {
      ...
      if (mutation.target.classList.contains("active")) {
        console.log('Element now has "new-class"');
      } else {
        console.log('Element no longer has "new-class"');
      }
    }
  }
};

// Options for the observer (observe attributes)
const config = { attributes: true };

// 2. Create an observer instance linked to the callback function
const observer = new MutationObserver(callback);

// 3. Start observing the target node for configured mutations
