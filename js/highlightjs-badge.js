...
(function (global, factory) {
  ...
  if (typeof highlightJsBadgeAutoLoad !== 'boolean') var highlightJsBadgeAutoLoad = false;
  ...
  function highlightJsBadge(opt) {
    ...
    function addCodeBadge() {
      ...
      for (var index = 0; index < $codes.length; index++) {
        ...
        else if (lang == 'txt') lang = 'text';

        var html = hudText.replace('{{language}}', lang).replace('{{copyIconClass}}', options.copyIconClass).trim();
        ...
        if (options.copyIconContent) $newHud.querySelector('.code-badge-copy-icon').innerText = options.copyIconContent;
    ...
    function copyCodeToClipboard(e) {
      ...
      if (options.onBeforeCodeCopied) text = options.onBeforeCodeCopied(text, $code);
    ...
    function swapIcons($code) {
      ...
      for (var i = 0; i < copyIcons.length; i++) $fa.classList.remove(copyIcons[i]);

      for (var i = 0; i < checkIcons.length; i++) $fa.classList.add(checkIcons[i]);

      setTimeout(function () {
        $fa.innerText = options.copyIconContent;

        for (var i = 0; i < checkIcons.length; i++) $fa.classList.remove(checkIcons[i]);
        for (var i = 0; i < copyIcons.length; i++) $fa.classList.add(copyIcons[i]);
      }, 2000);
    }
  ...
  // module export
  if (window.module && window.module.exports) window.module.exports.highlightJsBadge = highlightJsBadge;
